            ...
            <div class="flex justify-between h-16 items-center">
                <div class="flex items-center">
                    <a href="#" class="flex items-center gap-2">
                        <svg class="w-8 h-8 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"></path></svg>
                ...
                <div class="hidden md:flex space-x-8 items-center">
                    ...
                    @if (auth()->check())
                        <a href="{{ route('filament.admin.pages.dashboard') }}" class="font-semibold text-primary hover:text-amber-700">
                            Ke Dashboard
                        </a>
                    @else
                        {{-- Tombol Login Filament --}}
                        <a href="{{ route('filament.admin.auth.login') }}" class="text-gray-700 hover:text-primary font-semibold transition">
                            Masuk
                        </a>

                        {{-- Cek apakah route register filament ada (jika diaktifkan di Provider) --}}
                        @if (Route::has('filament.admin.auth.register'))
                            <a href="{{ route('filament.admin.auth.register') }}" class="bg-primary hover:bg-amber-700 text-white px-4 py-2 rounded-lg font-medium transition shadow-lg shadow-amber-500/30">
                                Daftar
                            </a>
                        @endif
                    @endif
    ...
    <section class="pt-32 pb-20 bg-white overflow-hidden">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <h1 class="text-4xl md:text-6xl font-extrabold text-gray-900 mb-6 leading-tight">
                Kelola Toko & Stok <br>
                <span class="text-primary bg-clip-text text-transparent bg-gradient-to-r from-amber-500 to-orange-600">Tanpa Ribet</span>
            </h1>
            ...
            <div class="flex justify-center gap-4 flex-col sm:flex-row items-center">
                @if(auth()->check())
                    <a href="{{ route('filament.admin.pages.dashboard') }}" class="px-8 py-3 bg-primary text-white rounded-xl font-bold text-lg hover:bg-amber-700 transition shadow-xl w-full sm:w-auto">
                        Buka Aplikasi
                    </a>
                @else
                    <a href="{{ route('filament.admin.auth.login') }}" class="px-8 py-3 bg-primary text-white rounded-xl font-bold text-lg hover:bg-amber-700 transition shadow-xl shadow-amber-500/20 w-full sm:w-auto transform hover:-translate-y-1">
                        Login Admin
                    </a>
                    
                    @if (Route::has('filament.admin.auth.register'))
                        <a href="{{ route('filament.admin.auth.register') }}" class="px-8 py-3 bg-white text-gray-700 border border-gray-200 rounded-xl font-bold text-lg hover:bg-gray-50 transition w-full sm:w-auto">
                            Daftar Baru
                        </a>
                    @endif
                @endif
            ...
            <div class="mt-16 relative rounded-xl bg-gray-900 p-2 shadow-2xl mx-auto max-w-5xl ring-1 ring-gray-900/10">
                <div class="bg-gray-800 rounded-lg overflow-hidden aspect-video relative flex items-center justify-center">
